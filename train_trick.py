...
from models import DenseNet121, DenseNet161, DenseNet161_trick
...
def train(args):
    ...
    # Load TRICK dataset
    datasetTrain_trick = CheXpertDualMoreBalancedDataSet('u_0_train_trick.csv', get_transforms('train', 320), policy='zeroes')
    datasetTest_front = CheXpertDataSet('u_0_test_trick_new.csv', get_transforms('test', 320), policy='zeroes')
    datasetVal_front = CheXpertDataSet('u_0_val_trick_new.csv', get_transforms('val', 320), policy='zeroes')
    ...
    u_0_val_DL_front = DataLoader(dataset=datasetVal_front, batch_size=320, shuffle=True, num_workers=num_workers, pin_memory=True)
    u_0_test_DL_front = DataLoader(dataset=datasetTest_front, batch_size=320, shuffle=True, num_workers=num_workers, pin_memory=True)
        
    ### Training
    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    nnClassCount = 1
    u_0_trick_model_dense161 = DenseNet161_trick(nnClassCount).to(device) # Step 0: Initialize global model and load the model
    ...
    # increase LR
    params, records = CheXpertTrainer_Asymmetric.train(u_0_trick_model_dense161, u_0_train_DL_trick, u_0_val_DL_front,
